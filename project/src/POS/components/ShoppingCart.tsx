  ...
  return (
    <div className="h-full flex flex-col cart-panel">
      ...
      <div className="bg-white border-b border-gray-200 p-4 md:p-6 shadow-sm">
        <div className="flex items-center justify-between mb-3 md:mb-4">
          <div className="flex items-center gap-2 md:gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs md:text-sm text-gray-500">{cart.length} item{cart.length !== 1 ? 's' : ''}</p>
            </div>
          </div>
          <button
            onClick={onClearCart}
            className="text-red-600 hover:text-red-700 active:bg-red-50 text-xs md:text-sm font-semibold px-2 md:px-3 py-2 rounded-lg transition-colors touch-button flex-shrink-0"
        ...
        {/* Add Customer Button */}
        <button className="w-full bg-emerald-50 border-2 border-dashed border-emerald-200 text-emerald-700 py-2.5 md:py-3 px-3 md:px-4 rounded-xl hover:bg-emerald-100 hover:border-emerald-300 active:bg-emerald-200 transition-colors font-semibold flex items-center justify-center gap-2 touch-button text-sm md:text-base">
          <User className="w-4 h-4 md:w-5 md:h-5" />
          <span className="truncate">{selectedCustomer ? 'Change Customer' : '+ Add Customer'}</span>
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto bg-gray-50 custom-scrollbar">
        <div className="p-3 md:p-4 space-y-2 md:space-y-3">
          {cart.map(item => (
            <div key={item.id} className="bg-white border border-gray-200 rounded-xl p-3 md:p-4 shadow-sm hover:shadow-md transition-shadow">
              <div className="flex items-start justify-between mb-3 md:mb-4 gap-2">
                <div className="flex-1 min-w-0">
                  <h3 className="font-semibold text-gray-900 text-sm md:text-lg mb-1 line-clamp-2">{item.product.name}</h3>
                  <p className="text-xs md:text-sm text-gray-500 font-mono truncate">SKU: {item.product.sku}</p>
                ...
                <button
                  ...
                  className="text-red-500 hover:text-red-700 active:bg-red-50 p-1.5 md:p-2 rounded-lg transition-colors touch-button flex-shrink-0"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
                  ...
                  <div className="quantity-control">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (item.selectedUnit?.is_base_unit && !Number.isInteger(val)) return;
                        handleQuantityChange(item.id, val);
                      }}
                      className="w-20 md:w-24 px-2 md:px-3 py-2 border border-gray-300 rounded-lg text-center font-semibold text-sm md:text-base focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500"
                    />
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm md:text-base font-semibold focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500"
                    placeholder="0.00"
                  />
                </div>
              </div>
      ...
      <div className="bg-white border-t border-gray-200 p-4 md:p-6 shadow-lg">
        <div className="space-y-2 md:space-y-3">
          ...
          <div className="border-t border-gray-200 pt-2 md:pt-3">
            <div className="flex justify-between items-center">
              ...
              <span className="text-xl md:text-2xl font-bold text-emerald-600">{formatPrice(total)}</span>
            </div>
          </div>
        </div>
        ...
        <button
          ...
          className="w-full mt-4 md:mt-6 bg-emerald-600 text-white py-3 md:py-4 px-4 md:px-6 rounded-xl font-bold text-base md:text-lg hover:bg-emerald-700 active:bg-emerald-800 active:scale-95 transition-all duration-200 shadow-lg hover:shadow-xl touch-button"
        >
          Proceed to Payment
        </button>
      </div>
