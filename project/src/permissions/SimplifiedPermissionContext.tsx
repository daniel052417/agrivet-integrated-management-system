...
interface SimplifiedPermissionContextType {
  // Current user info
...
const SimplifiedPermissionContext = createContext<SimplifiedPermissionContextType | undefined>(undefined);

interface SimplifiedPermissionProviderProps {
  children: ReactNode;
...
export const SimplifiedPermissionProvider: React.FC<SimplifiedPermissionProviderProps> = ({ children, user }) => {
  ...
  const contextValue: SimplifiedPermissionContextType = {
    currentUser,
    currentRole,
    userPermissions,
    hasPermission,
  ...
  return (
    <SimplifiedPermissionContext.Provider value={contextValue}>
      {children}
    </SimplifiedPermissionContext.Provider>
  );
};

export const useSimplifiedPermissions = (): SimplifiedPermissionContextType => {
  const context = useContext(SimplifiedPermissionContext);
  if (context === undefined) {
    throw new Error('useSimplifiedPermissions must be used within a SimplifiedPermissionProvider');
  }
  return context;
};
