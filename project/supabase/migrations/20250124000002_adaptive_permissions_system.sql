-- Adaptive Permissions System Migration
-- This migration adapts to the actual database schema structure

-- 1. First, let's check and fix the roles table structure
DO $$
BEGIN
    -- Check if roles table has id column (UUID) or role_id column (SERIAL)
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'roles' AND column_name = 'id' AND data_type = 'uuid'
    ) THEN
        -- Roles table uses UUID id column
        RAISE NOTICE 'Roles table uses UUID id column';
    ...
    ELSIF EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'roles' AND column_name = 'role_id' AND data_type = 'integer'
    ) THEN
        -- Roles table uses SERIAL role_id column
        RAISE NOTICE 'Roles table uses SERIAL role_id column';
    ...
    ELSE
        ...
        RAISE NOTICE 'Creating roles table';
        CREATE TABLE roles (
...
-- 3. Create new permissions table (replacing the old one)
...
-- 4. Create role_permissions junction table with adaptive foreign key
DO $$
BEGIN
    -- Check if roles table uses id (UUID) or role_id (SERIAL)
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'roles' AND column_name = 'id' AND data_type = 'uuid'
    ) THEN
        -- Create role_permissions with UUID reference
    ...
    ELSE
        -- Create role_permissions with INTEGER reference
        CREATE TABLE role_permissions (
            id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
-- 5. Create user_roles table with adaptive foreign key
DO $$
BEGIN
    -- Check if users table uses id (UUID) or user_id (SERIAL)
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'users' AND column_name = 'id' AND data_type = 'uuid'
    ) THEN
        -- Users table uses UUID id column
        IF EXISTS (
            SELECT 1 FROM information_schema.columns 
            WHERE table_name = 'roles' AND column_name = 'id' AND data_type = 'uuid'
        ) THEN
            -- Both users and roles use UUID
        ...
        ELSE
            -- Users use UUID, roles use INTEGER
            CREATE TABLE user_roles (
                id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
                user_id UUID REFERENCES users(id) ON DELETE CASCADE,
                role_id INTEGER REFERENCES roles(role_id) ON DELETE CASCADE,
        ...
        END IF;
    ELSE
        -- Users table uses SERIAL user_id column
        IF EXISTS (
            SELECT 1 FROM information_schema.columns 
            WHERE table_name = 'roles' AND column_name = 'id' AND data_type = 'uuid'
        ) THEN
            -- Users use INTEGER, roles use UUID
            CREATE TABLE user_roles (
                id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
        ...
        ELSE
            -- Both users and roles use INTEGER
            CREATE TABLE user_roles (
                id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
                user_id INTEGER REFERENCES users(user_id) ON DELETE CASCADE,
                role_id INTEGER REFERENCES roles(role_id) ON DELETE CASCADE,
...
-- 9. Assign permissions to roles (adaptive based on table structure)
DO $$
BEGIN
    -- Check if roles table uses id (UUID) or role_id (SERIAL)
    IF EXISTS (
...
-- 11. Create function to get user permissions (adaptive)
...
CREATE POLICY "Admins can manage roles" ON roles
    FOR ALL USING (
        EXISTS (
            ...
            JOIN roles r ON ur.role_id = r.id OR ur.role_id = r.role_id
