-- Add batch_no and expiration_date fields to inventory views
-- These fields are stored in the inventory table, not the products table

-- Update inventory_with_units view to include batch_no and expiration_date
...
GROUP BY 
    p.id, p.sku, p.name, p.image_url, p.cost, p.is_active, p.barcode, p.brand, p.supplier_id, p.description,
...
-- Update inventory_management_view to include batch_no and expiration_date
...
-- Create inventory_management view (alias) that points to inventory_management_view
-- This ensures compatibility with code that queries 'inventory_management'
CREATE OR REPLACE VIEW inventory_management AS
SELECT * FROM inventory_management_view;
