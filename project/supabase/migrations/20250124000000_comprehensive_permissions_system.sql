    ...
    SELECT 
        ...
        CASE 
            ...
            WHEN EXISTS (
                ...
                WHERE up.permission_name = ca.required_permission
            ) THEN true
            ELSE false
        END as has_access
...
BEGIN
    RETURN EXISTS (
        ...
        WHERE permission_name = user_has_permission.permission_name
    );
END;
$$ LANGUAGE plpgsql;
...
CREATE POLICY "Admins can manage roles" ON roles
    FOR ALL USING (
        EXISTS (
            ...
            AND r.role_name IN ('super-admin', 'hr-admin', 'marketing-admin')
            AND ur.is_active = true
        )
    );
